function multiply(a, b) {
  ...
  let total = []  //先把乘出來用的數字用陣列存起來，total 來裝很多行 line  
...
function add(arr){
	...
	for (let i = 0 ; i < container.length; i++){
		container[i] = carryDigit + container[i]			//處理進位的問題，從個位數開始。tmp 為前一位數的進位。
		ans.push(container[i] % 10)					//存進 ans
		carryDigit = Math.floor(container[i] / 10)			//將進位的數字存進 tmp
		if (i === container.length - 1 && carryDigit !== 0)ans.push(carryDigit) //跑完container但tmp不是 0，代表要進位
	}

/*  下面這段在刪除數字前面的 0 ，如輸入是 05,03 會變成 015。這段程式把前面的 0 給刪掉，變成 15。*/
	let zeroCounter = NaN
	for (let i = ans.length - 1; i >= 0; i--){
		if (ans[i] === 0) zeroCounter = i
		else break
	}
	if (zeroCounter !== NaN) ans.splice(zeroCounter,ans.length - zeroCounter)

/* 將數字陣列轉換成字串  */
	let ansStr = ans.reverse()
	return ansStr.join('')
}
